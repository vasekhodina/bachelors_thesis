\documentclass[printed,color,table,oneside,nolot,nolof]{fithesis}
...
\thesissetup{
	  date = \the\year/\the\month/\the\day,
		...
		TeXkeywords = {vizualizace, instalace, rozdělení disků, Linux, LVM, RAID, graf}, 
}
\thesislong{abstract}{
	Práce popisuje vývoj nástroje pro vizualizaci dat s použitím knihovny Blivet, nástroje pro správu blokových zařízení.
...
Tato práce vznikla ve spolupráci se společností Red Hat Czech s.r.o., za co jí též patří můj dík.
}
\usepackage{csquotes}
...
\chapter{Závěr}
  Hlavním cílem mé práce byla vizualizace dat z~Blivetu do samostatného okna GTK aplikace a~zobrazení připravených změn. Dalším cílem byla příprava funkcí pro snadnou integraci
	do dalších programů.   

	Vizualizační program čte data z~knihovny Blivet a~s~pomocí knihov\-ny Graphviz z~nich vytváří graf. Ten zobrazuje s~pomocí programu WebKit ve vlastním okně aplikace. Program lze
